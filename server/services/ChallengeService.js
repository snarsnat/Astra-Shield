...
export class ChallengeService {
  constructor(options = {}) {
    ...
    // Challenge patterns (for different difficulty levels)
    this.patterns = {
      pulse: {
    ...
    // Recent challenge IDs for replay protection
  ...
  generateVerificationToken(challengeId) {
    ...
    const signature = crypto
      .createHmac('sha256', 'astra-verification-secret')
      .update(payload)
      .digest('base64url');
  ...
  formatChallengeForClient(challenge) {
    const { data, ...rest } = challenge;

    // Don't send sensitive data to client
    const clientData = { ...data };

    // For PoW, send the prefix but not the solution
    if (data.type === 'proof_of_work') {
      clientData.prefix = data.prefix;
      clientData.difficulty = data.difficulty;
    }
